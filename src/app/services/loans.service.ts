...
export class LoansService {
  ...
  create(loan: { userId: number, bookId: number }) {
    return this.http.post(this.api, loan);
  }
  updateStatus(id: number, status: string, dataDevolucao?: string) {
    return this.http.put(`${this.api}/${id}`, { status, dataDevolucao });
  }
  listByUser(userId: number) {
    return this.http.get<Loan[]>(`${this.api}/user/${userId}`);
  }
}
